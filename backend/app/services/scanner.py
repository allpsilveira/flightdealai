...
# 300 statute miles = 483 km straight-line radius.
# The user enters one home airport; anything within this radius is auto-included.
NEARBY_RADIUS_KM = 483
...
def expand_origins_nearby(
    origins: list[str],
    max_nearby_per_origin: int = 4,
) -> list[str]:
    """
    Auto-expand the user's home airport(s) with nearby commercial airports
    within NEARBY_RADIUS_KM (300 miles). Sorted by distance — closest first.

    max_nearby_per_origin caps how many additional airports are added per
    home airport so a dense region (northeast US corridor) doesn't explode
    the scan count. Default 4 → at most 5 airports total per home airport.
    """
    ...
    for origin_code in origins:
        ...
        for dist, ap in nearby:
            if added >= max_nearby_per_origin:
                break
...
async def scan_route(
    ...
    origins here are already the expanded set (home airport + nearby within 300 miles),
    computed by expand_origins_nearby() before this is called.
